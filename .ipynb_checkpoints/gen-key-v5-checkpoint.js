...
'use strict';
/**
 * gen-key-v5.js — Offline V5 base key generator
 ...
 *
 * Usage:
 *   node gen-key-v5.js starter lifetime
 *   node gen-key-v5.js growth lifetime
 *   node gen-key-v5.js pro monthly
 *   node gen-key-v5.js pro yearly
 *   node gen-key-v5.js pro lifetime
 *   node gen-key-v5.js enterprise monthly
 *   node gen-key-v5.js enterprise yearly
 *   node gen-key-v5.js trial
 *   node gen-key-v5.js --owner
 *   node gen-key-v5.js --list
 */
...
/**
 * Compute the 9-char base-36 SIG for a base key.
 * payload: "V5|<TIER_CODE>|<BILLING_CODE>|<DURATION_B36>"
 */
function computeSig(tierCode, billingCode, durationB36) {
...
function buildKey(tier, billing) {
  ...
  const sig         = computeSig(tierCode, billingCode, durationB36);
  const key         = `V5${tierCode}${billingCode}${durationB36}${sig}`;
  if (key.length !== 16) {
...
/**
 * Print a formatted table of all 9 base keys.
 */
function printList() {
  console.log('\n═══════════════════════════════════════════════════════════════════════════');
  console.log(' V5 BASE KEY CATALOGUE — جدول المفاتيح الأساسية');
  console.log('═══════════════════════════════════════════════════════════════════════════');
  console.log(
  ...
  console.log('═══════════════════════════════════════════════════════════════════════════\n');
  console.log(' ⚠️  احتفظ بهذه المفاتيح بشكل آمن. لا ترسلها عبر قنوات غير مشفرة.');
