...
function migrateForSync(db) {
    ...
    // 1. Create Sync Engine Tracking Tables
    db.exec(`
    ...
    // Register native JS UUID generator for SQLite
    try {
    ...
    for (const table of syncTables) {
        // 2. Safe schema checks using PRAGMA table_info
        ...
        // This generates a UUID for old rows and sets their updated_at to the current Unix timestamp
        db.exec(`
            UPDATE ${table} 
        ...
        // 3.5. Enforce uniqueness on sync_id via index (SQLite does not allow adding UNIQUE columns to existing tables)
        db.exec(`CREATE UNIQUE INDEX IF NOT EXISTS idx_${table}_sync_id ON ${table}(sync_id);`);

        // 4. Create AFTER INSERT trigger
        // Automatically populates sync_id and updated_at if the frontend didn't provide them
        db.exec(`
        ...
        // MAGIC TRICK: "WHEN NEW.updated_at = OLD.updated_at" 
        // Ensures that if the local React UI updates a row, we bump the timestamp.
        // BUT if the Sync Engine applies a remote update, it explicitly sets the remote updated_at,
        // so NEW != OLD, and this trigger bypasses itself, preserving the true Master timestamp!
        db.exec(`
...
function initDatabase(userDataPath) {
    ...
    // Migration: ensure new sale columns exist
    ...
    // Migration: ensure promotion columns exist
    ...
    // Migration for existing databases — ensure 2202 and 1103 exist (3-A)
    try { 
    ...
    // SAFE MIGRATIONS (won't throw if column already exists)
    ...
    // FIX: Track loyalty points redeemed per sale so void can restore them
    ...
    // [C-2] Per-product tax category for ZATCA compliance
    safe(`ALTER TABLE products ADD COLUMN tax_category TEXT DEFAULT 'S'`);

    // [C-1] B2B clearance support
    ...
    // [H-1] Certificate expiry monitoring
    safe(`ALTER TABLE zatca_device ADD COLUMN cert_expires_at DATETIME`);
    // New migrations for pre-launch fixes
    safe(`ALTER TABLE purchase_orders ADD COLUMN vat_included INTEGER DEFAULT 1`);   // 3-B
    safe(`ALTER TABLE purchase_orders ADD COLUMN vat_amount REAL DEFAULT 0`);         // 3-B
    safe(`ALTER TABLE expenditures ADD COLUMN status TEXT DEFAULT 'active'`);         // 5-A soft-delete
    ...
    // Backfill plain-text legacy PINs (detect non-hex = still plain)
    ...
    // ─────────────────────────────────────────────
    // EMERGENCY PIN RESET (v2026 Core fix)
    // ─────────────────────────────────────────────
    try {
        console.log('[Auth] Checking for emergency PIN resets...');
        // Case-insensitive search for Admin and Cashier roles
    ...
    // Execute Phase 1 Sync Migrations safely
    console.log('Running Sync Engine Migrations...');
    migrateForSync(db);

    console.log('Database connected and ready.');

    // ── Phase 1: Initialize accounting engine (migrations + CoA seed) ──
...
function getPurchaseOrders() {
    // FIX BUG 2: Use LEFT JOIN so orders with deleted suppliers still show
    return db.prepare(`
...
function receivePurchaseOrder(id) {
    ...
    // Read cost_method from settings (1-B)
    ...
    return db.transaction(() => {
        ...
        // Ledger Entry with conditional VAT split (3-B)
        const vatIncluded = purchase.vat_included !== 0; // default true
        ...
        // Store computed vat_amount on the PO for reference
...
function saveSale(saleData) {
    const {
        ...
        // FIX: Accept loyalty_points_redeemed from the frontend so we can
        // deduct points atomically inside this transaction and record the
        // amount for later void-reversal.
        loyalty_points_redeemed = 0,
        discount_type = 'normal',
        is_agreed_total = 0,
    } = saleData;

    // 2-F: Enforce open shift
    ...
    // Sanitize the points value: must be a non-negative integer
    ...
    const runTx = db.transaction(() => {
        ...
        const newIcv = db.prepare('SELECT current_icv FROM zatca_device WHERE id = ?').get(device.id).current_icv; // [H-2] read-after-write
        ...
        if (customer_id) {
            ...
            // FIX: Deduct redeemed points atomically here, within the same
            // transaction as the sale insert. Using MAX(0, ...) prevents the
            // balance from going negative due to any race condition.
            // Net change = earnedPts (for this purchase) − redeemedPts.
            // We do two separate updates for clarity and auditability.
            db.prepare(`
                UPDATE customers
        ...
        // 3-A: Correct account selection per payment method
        // Credit/آجل → Debit Receivables (1103 ذمم مدينة — Asset)
        // Card (STC/Card) → Debit Bank (1102)
        // Cash → Debit Cash (1101)
        let paymentAcct;
        if (payment === 'آجل' || payment === 'Credit') {
            paymentAcct = 1103; // Accounts Receivable (Asset) — fixed from 2101
        ...
        // Perpetual Inventory: Dr COGS (5101), Cr Inventory (1201)
        let totalCOGS = 0;
        const history_stmt = db.prepare(`INSERT INTO stock_history (product_id, change_amount, reason, reference_id) VALUES (?,?,?,?)`);

        for (const item of items) {
            if (!item.ID || item.IsService) {
                // Services don't affect stock; still record in sales_items
            ...
            if (row) {
                if (!row.is_service && item.Qty > 0 && row.stock < item.Qty) {
                    // 1-F: Structured error payload for frontend
                ...
                if (!row.is_service) {
                    // CREDIT NOTE FIX: negative Qty = return → add back to stock
                    // Positive Qty = sale → deduct from stock
                    db.prepare('UPDATE products SET stock=stock-? WHERE id=?').run(item.Qty, item.ID);
                    history_stmt.run(item.ID, -item.Qty,
                        item.Qty < 0 ? 'credit_note_return' : 'sale', invoice);
                }
        ...
        // ── P-018: Post COGS using FIFO/AVCO via computeSaleCOGS() ────────────
        // Falls back to legacy product.cost if no inventory_batches exist.
        ...
        for (const item of items) {
            ...
            try {
                ...
                // If no batch data, fall back to product cost already computed in totalCOGS
            }
        }
        // Use P2 costing if we got a meaningful result, otherwise fall back
        const finalCOGS = totalCOGS_p2 > 0 ? totalCOGS_p2 : totalCOGS;

        if (finalCOGS > 0) {
            // Post to legacy ledger_entries for backward compat
            recordTransaction([
                { code: 5101, debit: finalCOGS, credit: 0 },
                { code: 1201, debit: 0,         credit: finalCOGS }
            ], `تكلفة بضاعة مباعة: ${invoice}`, invoice);
            // Also post to double-entry journal_entries (new accounting engine)
            try {
            ...
            } catch(_) {} // Non-fatal — legacy ledger already recorded
        }

        // ── P-013: Post revenue JE to double-entry engine ────────────────────
        // Also handles reverse-charge VAT per product tax_category.
        try {
            const saleDate = new Date().toISOString().split('T')[0];
            // Determine debit account
            let drAcct = 1111; // Cash
            ...
            // Reverse-charge VAT: for each item with tax_category = 'RC',
            // post Dr Input-VAT / Cr Output-VAT for the VAT component
            for (const item of items) {
                if (!item.ID) continue;
            ...
            // Non-fatal — legacy ledger_entries already recorded above
...
function voidSale(invoiceId, reason = 'لم يُحدد') {
    ...
    // FIX: Read back how many loyalty points were redeemed on this sale so
    // we can restore them to the customer's balance on void.
    ...
    db.transaction(() => {
        ...
        // FIX: Restore loyalty points that were redeemed on this sale,
        // and reverse the spend that was credited when the sale was saved.
        // We also remove the earned points (1 per SAR 10) since the sale is void.
        if (sale.customer_id) {
            ...
            // Restore the redeemed points (customer gets them back)
        ...
        // 3-A: Reverse using correct account (1103 for credit, not 2101)
...
function getSalesHistory(filters = {}) {
    ...
    // FIX: Normalize both 'void' and 'voided' status values
    if (filters.status) { 
...
function addExpenditure(data) {
    ...
    return db.transaction(() => {
        ...
        // Dr Expense (5201), Dr VAT Input (2202), Cr Cash (1101)
        recordTransaction([
...
// 5-A: Soft-delete with audit + ledger reversal (1-E)
function deleteExpenditure(id) {
    ...
    return db.transaction(() => {
        // Reverse ledger: Dr Cash (1101), Cr Expense (5201), Cr VAT Input (2202)
...
function getFinancialReport({ startDate, endDate } = {}) {
    ...
    // FIX BUG 10: Pull period-specific totals from ledger_entries, NOT from cumulative account balances
...
// NOTE: redeemLoyaltyPoints is kept for backward compatibility (e.g. called
// directly from admin tools), but is no longer called by the POS checkout
// flow — points are now deducted atomically inside saveSale().
function redeemLoyaltyPoints(customerId, points) {
    ...
    return { success: true, discount: points * 0.1 }; // 1 point = 0.10 SAR
}
...
function createReturn(invoiceId, returnItems) {
    ...
    return db.transaction(() => {
        ...
        // Ledger: reverse revenue portion
