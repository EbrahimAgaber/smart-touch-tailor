 ...
 */
function generateVAT311XML(param1, param2) {
    let boxes, settings, period_start, period_end;

    if (param1 && typeof param1 === 'object' && param1.boxes) {
        boxes = param1.boxes;
        settings = param1.settings;
        period_start = param1.period_start || param1.startDate;
        period_end = param1.period_end || param1.endDate;
    } else if (param1 && typeof param1 === 'object') {
        period_start = param1.startDate || param1.period_start;
        period_end = param1.endDate || param1.period_end;
    } else {
        period_start = param1;
        period_end = param2;
    }
    ...
    period_start = period_start || todayStr;
    period_end = period_end || todayStr;

    if (!boxes && typeof p2 !== 'undefined' && p2.getVATReturnBoxes) {
        try { boxes = p2.getVATReturnBoxes(period_start, period_end); } catch (e) {}
    }
    if (!settings && typeof _db !== 'undefined') {
        try {
            const row = _db.prepare("SELECT value FROM settings WHERE key = 'business_settings'").get();
            if (row && row.value) settings = JSON.parse(row.value);
        } catch (e) {}
    }
    settings = settings || {};

    const vatNo    = settings.vat_number || settings.tax_number || '300000000000003';
    const bizName  = settings.business_name_ar || settings.business_name || 'منشأة تجارية';
    const crn      = settings.crn || '1010000000';
    ...
    const b = {
        box1_taxable: rawB.box1_taxable ?? rawB.box1_standard_amount ?? 0,
        box1_tax:     rawB.box1_tax     ?? rawB.box1_standard_vat    ?? 0,
        box2_taxable: rawB.box2_taxable ?? rawB.box2_zero_rated     ?? 0,
        box3_taxable: rawB.box3_taxable ?? rawB.box3_exempt         ?? 0,
        box4_taxable: rawB.box4_taxable ?? 0,
        box4_tax:     rawB.box4_tax     ?? 0,
        box5_vat_sales: rawB.box5_vat_sales ?? rawB.box1_standard_vat ?? 0,
        box6_taxable: rawB.box6_taxable ?? rawB.box5_input_amount   ?? 0,
        box6_tax:     rawB.box6_tax     ?? rawB.box5_input_vat      ?? 0,
        box7_taxable: rawB.box7_taxable ?? 0,
        box7_tax:     rawB.box7_tax     ?? 0,
        box8_taxable: rawB.box8_taxable ?? 0,
        box8_tax:     rawB.box8_tax     ?? 0,
        box9_total_input: rawB.box9_total_input ?? rawB.box5_input_vat ?? 0,
        net_vat:      rawB.net_vat      ?? rawB.box9_vat_due        ?? 0,
    };
...
<VATReturn xmlns="urn:gov:sa:zatca:vat:return:v1"
  ...
  <TaxableSupplies>
    ...
    <Box1>
      ...
      <TaxableAmount>${d(b.box1_taxable)}</TaxableAmount>
      <TaxAmount>${d(b.box1_tax)}</TaxAmount>
    </Box1>
    <!-- Box 2: Zero-rated domestic supplies -->
    <Box2>
      <Description>الإيرادات بمعدل الصفر — المحلية</Description>
      <TaxableAmount>${d(b.box2_taxable || 0)}</TaxableAmount>
      <TaxAmount>0.00</TaxAmount>
    </Box2>
    <!-- Box 3: Exempt supplies -->
    <Box3>
      <Description>الإيرادات المعفاة</Description>
      <TaxableAmount>${d(b.box3_taxable || 0)}</TaxableAmount>
      <TaxAmount>0.00</TaxAmount>
    </Box3>
    <!-- Box 4: GCC intra-community supplies -->
    <Box4>
      <Description>التوريدات داخل دول مجلس التعاون</Description>
      <TaxableAmount>${d(b.box4_taxable || 0)}</TaxableAmount>
      <TaxAmount>${d(b.box4_tax || 0)}</TaxAmount>
    </Box4>
    <!-- Box 5: Total VAT on sales -->
    <Box5>
      <Description>إجمالي ضريبة القيمة المضافة على المبيعات</Description>
      <TaxAmount>${d(b.box5_vat_sales || (parseFloat(b.box1_tax || 0) + parseFloat(b.box4_tax || 0)))}</TaxAmount>
    </Box5>
  </TaxableSupplies>
  ...
  <TaxablePurchases>
    ...
    <Box6>
      ...
      <TaxableAmount>${d(b.box6_taxable || 0)}</TaxableAmount>
      <TaxAmount>${d(b.box6_tax || 0)}</TaxAmount>
    </Box6>
    <!-- Box 7: Imports subject to VAT from Customs -->
    <Box7>
      <Description>الاستيراد الخاضع للضريبة (جمارك)</Description>
      <TaxableAmount>${d(b.box7_taxable || 0)}</TaxableAmount>
      <TaxAmount>${d(b.box7_tax || 0)}</TaxAmount>
    </Box7>
    <!-- Box 8: Imports via reverse charge -->
    <Box8>
      <Description>الاستيراد عبر الاحتساب العكسي</Description>
      <TaxableAmount>${d(b.box8_taxable || 0)}</TaxableAmount>
      <TaxAmount>${d(b.box8_tax || 0)}</TaxAmount>
    </Box8>
    <!-- Box 9: Total input VAT available for deduction -->
    <Box9>
      <Description>إجمالي ضريبة المدخلات القابلة للخصم</Description>
      <TaxAmount>${d(b.box9_total_input || b.box6_tax || 0)}</TaxAmount>
    </Box9>
  </TaxablePurchases>
