...
if (!dbCode.includes('CREATE TABLE IF NOT EXISTS global_catalog')) {
    // Regex for robust matching across OS line endings
    dbCode = dbCode.replace(/unit TEXT DEFAULT 'وحدة'[\r\n\s]*\);[\r\n\s]*`\);/m, initDbReplacement);
}

const editItemTargetRegex = /function editItem\(item\) \{[\s\S]*?return item;\r?\n\}/m;
...
if (!dbCode.includes("db.prepare('INSERT INTO stock_history")) {
    dbCode = dbCode.replace(editItemTargetRegex, editItemReplacement);
}
