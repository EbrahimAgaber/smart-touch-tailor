...
export default function Pos() {
  ...
  const printReceiptData = async (invoiceObj, forceWidth) => {
    ...
    const qrWarning = !qrData
      ...
      : '';
    // AFTER (correct)
// qrData is a Base64-encoded binary TLV — decode it to raw bytes before
// passing to the QR encoder, which must encode the binary payload, not the ASCII text.
    let qrPayload = '';
    if (qrData) {
      try {
    // atob() converts base64 → binary string; that binary string is what QR must encode
        qrPayload = atob(qrData);
      } catch (e) {
        qrPayload = '';
      }
    }
    const svg = QRCode.generateSVG(qrPayload || '', 4);
