...
export default function MenuAdmin() {
  ...
  const handleImageUpload = async () => {
    ...
    try {
      ...
      alert(t('menu.form.image_error') + e.message);
    }
    setUploading(false);
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      alert(t('menu.form.save_error') + err.message);
    }
    setSaving(false);
  };
        ...
        <div style={{ background:'white', borderRadius:'24px', padding:'28px', border:'1px solid #f1f5f9', position:'sticky', top:0, maxHeight:'90vh', overflowY:'auto' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'24px' }}>
            <h3 style={{ fontWeight:'900', fontSize:'18px', color:'#0f172a' }}>
              {editId ? t('menu.form.edit_title') : t('menu.form.add_title')}
            </h3>
            {!editId && (
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'20px' }}>
            ...
            <div style={formSection}>
              <SectionHeader icon={<Info size={16}/>} label={t('menu.form.section_basic')} />
              <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
                <F label={t('menu.form.name_label')} value={form.Name} onChange={v => updateField('Name', v)} placeholder={t('menu.form.name_placeholder')} />
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <F label={t('menu.form.price_label')} type="number" value={form.Price} onChange={v => updateField('Price', v)} placeholder="0.00" />
                  <F label={t('menu.form.category_label')} value={form.Category} onChange={v => updateField('Category', v)} placeholder={t('menu.form.category_placeholder')} />
                </div>
                <F label={t('menu.form.barcode_label')} value={form.Barcode} onChange={v => updateField('Barcode', v)} placeholder={t('menu.form.barcode_placeholder')} />
                ...
                <div style={{ display:'flex', alignItems:'center', gap:'20px', marginTop:'10px' }}>
                  ...
                  <div style={{ flex:1 }}>
                    ...
                    <button type="button" onClick={handleImageUpload} disabled={uploading}
                      ...
                      <Upload size={14}/> {uploading ? t('menu.form.uploading') : t('menu.form.choose_image')}
                    </button>
                    {form.Image && (
            ...
            <div style={formSection}>
              <SectionHeader icon={<Box size={16}/>} label={t('menu.form.section_inventory')} />
              <div className="flex flex-col gap-3">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <F label={t('menu.form.cost_label')} type="number" value={form.Cost} onChange={v => updateField('Cost', v)} placeholder="0.00" />
                  <div>
                    <label style={{ display:'block', marginBottom:'6px', fontWeight:'800', fontSize:'12px', color:'#64748b' }}>{t('menu.form.unit_label')}</label>
                    <input
                      list="zatca-units"
                      value={form.Unit || ''}
                      onChange={e => updateField('Unit', e.target.value)}
                      placeholder={t('menu.form.unit_placeholder')}
                ...
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px', background:'#f8fafc', borderRadius:'14px', border:'1px solid #e2e8f0' }}>
                  <span style={{ fontSize:'13px', fontWeight:'700', color:'#475569' }}>{t('menu.form.is_service')}</span>
                ...
                {!form.IsService && (
                  <div className="grid grid-cols-2 gap-3 mt-1">
                    <F label={t('menu.form.current_stock')} type="number" value={form.Stock} onChange={v => updateField('Stock', v)} />
                    <F label={t('menu.form.min_stock')} type="number" value={form.MinStockLevel} onChange={v => updateField('MinStockLevel', v)} />
                  </div>
                )}
              </div>
            </div>
            ...
            {!form.IsService && (
              <div style={formSection}>
                <SectionHeader icon={<Layers size={16}/>} label={t('menu.form.section_bulk')} />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <F label={t('menu.form.bulk_name')} value={form.BulkUnitName} onChange={v => updateField('BulkUnitName', v)} placeholder={t('menu.form.bulk_name_placeholder')} />
                  <F label={t('menu.form.bulk_size')} type="number" value={form.BulkUnitSize} onChange={v => updateField('BulkUnitSize', v)} />
                </div>
              </div>
            )}
            ...
            <div style={formSection}>
              <SectionHeader icon={<Settings size={16}/>} label={t('menu.form.section_advanced')} />
              <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
                <label style={{ display:'flex', alignItems:'center', gap:'10px', fontSize:'13px', fontWeight:'600', color:'#0f172a', cursor:'pointer' }}>
                  ...
                  {t('menu.form.requires_serial')}
                </label>
                <F label={t('menu.form.warranty_days')} type="number" value={form.Metadata.warranty_days} onChange={v => updateMeta('warranty_days', v)} />
              </div>
            </div>
          </div>
          ...
          <div style={{ display:'flex', gap:'12px', marginTop:'32px' }}>
            <button onClick={save} disabled={saving || !form.Name} style={{ ...actionBtn, background:'#3b82f6', color:'white', flex:2 }}>
              {saving ? t('menu.form.saving') : editId ? t('menu.form.update_btn') : t('menu.form.save_btn')}
            </button>
            {editId && (
        ...
        <div style={{ background:'white', borderRadius:'24px', padding:'28px', border:'1px solid #f1f5f9', display:'flex', flexDirection:'column', height:'100%', minHeight:0 }} className="max-lg:order-first">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <div style={{ display:'flex', alignItems:'center', gap:'12px' }}>
              ...
              <h3 style={{ fontWeight:'900', fontSize:'18px' }}>{t('menu.table.title', { count: items.length })}</h3>
            </div>
            <div style={{ position:'relative', width:'300px' }}>
              <Search size={16} style={{ position:'absolute', right:'14px', top:'50%', transform:'translateY(-50%)', color:'#94a3b8' }} />
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder={t('menu.table.search_placeholder')} style={{ width:'100%', padding:'12px 42px 12px 16px', borderRadius:'14px', border:'1px solid #e2e8f0', outline:'none', fontSize:'14px' }} />
            </div>
          </div>
            ...
            <table style={{ width:'100%', borderCollapse:'collapse', textAlign:'right' }}>
              <thead>
                <tr style={{ borderBottom:'2px solid #f1f5f9' }}>
                  <th style={TH}>{t('menu.table.product')}</th>
                  <th style={TH} className="max-sm:hidden">{t('menu.table.category')}</th>
                  <th style={TH} className="max-md:hidden">{t('menu.table.price_cost')}</th>
                  <th style={TH}>{t('menu.table.stock')}</th>
                  <th style={TH}>{t('menu.table.actions')}</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map(p => {
                    ...
                    <tr key={p.ID} style={{ borderBottom:'1px solid #f8fafc' }}>
                      <td style={TD}>
                        ...
                        <div style={{ fontSize:'11px', color:'#94a3b8', marginTop:'4px', display:'flex', alignItems:'center', gap:'4px' }}>
                          <Barcode size={10} /> {p.Barcode || t('menu.table.no_barcode')}
                        </div>
                      </td>
                      <td style={TD} className="max-sm:hidden"><Pill>{p.Category || t('menu.table.category_default')}</Pill></td>
                      <td style={TD} className="max-md:hidden">
                        <div style={{ fontWeight:'800', color:'#3b82f6' }}>SAR {parseFloat(p.Price || 0).toFixed(2)}</div>
                        <div style={{ fontSize:'11px', color:'#94a3b8' }}>{t('menu.table.cost_label')}{p.Cost ? p.Cost.toFixed(2) : '0.00'}</div>
                        {margin && <div style={{ fontSize:'10px', color: margin > 20 ? '#10b981' : '#f59e0b', fontWeight:'700' }}>{t('menu.table.margin_label')}{margin}%</div>}
                      </td>
                      <td style={TD}>
                        {p.IsService ? (
                          <Pill color="purple">{t('menu.table.service_label')}</Pill>
                        ) : (
                          <div style={{ display:'flex', alignItems:'center', gap:'6px' }}>
                            <div style={{ padding:'4px 10px', borderRadius:'10px', fontSize:'12px', fontWeight:'800', background: isLow ? '#fef2f2' : '#f0fdf4', color: isLow ? '#ef4444' : '#10b981' }}>
                              {p.Stock} {p.Unit || t('menu.table.unit_default')}
                            </div>
                      ...
                      <td style={TD}>
                        <div style={{ display:'flex', gap:'6px' }}>
                          <button onClick={() => setLabelProduct(p)} style={{ ...iconBtn, color:'#3b82f6' }} title={t('menu.history.title')}><Tag size={16} /></button>
                          <button onClick={() => openHistory(p)} style={iconBtn} title={t('menu.history.title')}><History size={16} /></button>
                          <button onClick={() => startEdit(p)} style={iconBtn}><Edit3 size={16} /></button>
                          <button onClick={() => del(p.ID)} style={{ ...iconBtn, color:'#ef4444' }}><Trash2 size={16} /></button>
                        </div>
                      </td>
                    </tr>
                  );
        ...
        <div style={overlay}>
          <div style={{ ...modal, maxWidth:'700px' }}>
            <div style={{ padding:'24px', borderBottom:'1px solid #f1f5f9', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              <div>
                <h3 style={{ fontWeight:'900', fontSize:'18px' }}>{t('menu.history.title')}</h3>
              ...
              <table style={{ width:'100%', borderCollapse:'collapse', textAlign:'right' }}>
                <thead>
                  <tr style={{ color:'#94a3b8', fontSize:'12px' }}>
                    <th style={{ padding:'12px' }}>{t('menu.history.product')}</th>
                    <th style={{ padding:'12px' }}>{t('menu.history.quantity')}</th>
                    <th style={{ padding:'12px' }}>{t('menu.history.reason')}</th>
                    <th style={{ padding:'12px' }}>{t('menu.history.reference')}</th>
                    <th style={{ padding:'12px' }}>{t('menu.history.date')}</th>
                  </tr>
                </thead>
                <tbody>
                  {history.map(h => (
                    <tr key={h.id} style={{ borderBottom:'1px solid #f8fafc' }}>
                      ...
                      <td style={{ padding:'12px', fontSize:'13px' }}>
                        {h.reason === 'sale' ? t('menu.history.reasons.sale') : h.reason === 'purchase' ? t('menu.history.reasons.purchase') : h.reason === 'adjustment' ? t('menu.history.reasons.adjustment') : h.reason}
                      </td>
                  ...
                  {history.length === 0 && (
                    <tr><td colSpan="5" style={{ textAlign:'center', padding:'40px', color:'#94a3b8' }}>{t('menu.history.no_history')}</td></tr>
                  )}
                </tbody>
              </table>
            </div>
