  ...
  return (
    <AppLayout title="إدارة الموردين">
      <div style={{ display:'flex', flexDirection:'column', gap:'24px', flex:1, minHeight:0 }}>
        ...
        {/* HEADER / CONTROLS */}
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', background:'white', padding:'16px 20px', borderRadius:'16px', border:'1px solid #f1f5f9', gap:'16px' }}>
        ...
        {loading ? (
          ...
          <div style={{ background:'white', padding:'80px', borderRadius:'20px', border:'1px dotted #cbd5e1', textAlign:'center', color:'#94a3b8' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(320px, 1fr))', gap:'20px', overflowY:'auto', flex:1, minHeight:0 }}>
            {filteredSuppliers.map(s => (
              <div key={s.id} style={{ background:'white', borderRadius:'20px', border:'1px solid #f1f5f9', padding:'20px', display:'flex', flexDirection:'column', gap:'14px', boxShadow:'0 1px 3px rgba(0,0,0,0.04)', transition:'transform 0.2s' }}
                ...
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
                  <div style={{ display:'flex', gap:'12px', alignItems:'center' }}>
                    ...
                    <div>
                      <h4 style={{ fontWeight:'800', fontSize:'16px', color:'#0f172a' }}>{s.name}</h4>
                      <div style={{ fontSize:'12px', color:'#64748b', fontWeight:'600' }}>{s.contact_person || 'بدون جهة اتصال'}</div>
                    </div>
                  </div>
...
function InfoItem({ icon, text, label }) {
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:'8px', fontSize:'13px' }}>
      ...
      <span style={{ color:'#64748b', fontSize:'11px', fontWeight:'600', minWidth:'80px' }}>{label}:</span>
      <span style={{ color:'#334155', fontWeight:'700' }}>{text}</span>
    </div>
  );
}
...
const fLabel = { display:'block', fontSize:'12px', fontWeight:'700', color:'#94a3b8', marginBottom:'6px' };
const fInput = { width:'100%', padding:'12px 14px', borderRadius:'12px', border:'1px solid #e2e8f0', fontSize:'14px', outline:'none', fontFamily:'inherit', background:'#fcfdfe', boxSizing:'border-box' };
